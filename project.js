...
document.querySelectorAll('.case-body > .scene-modes').forEach(viewer => {
  ...
  function draw() {
    ...
    label.textContent = images[Math.min(2, Math.floor(progress + .5))].dataset.sceneLabel;
  }
  function measure() {
...
const comparisonCards = [...document.querySelectorAll('.case-body > .scene-modes')]
  .filter(card => card.querySelector('.scene-mode-stage')?.getAttribute('aria-label')?.startsWith('Physical and Fortnite'));
if (comparisonCards.length > 1) {
  const gallery = document.createElement('section');
  gallery.className = 'comparison-gallery';
  gallery.setAttribute('aria-label', 'Physical exhibition and Fortnite comparisons');
  gallery.innerHTML = `<div class="comparison-sticky"><div class="comparison-viewport" tabindex="0" role="region" aria-label="Exhibition comparisons"><div class="comparison-track"></div></div><div class="comparison-controls"><span class="comparison-count"></span><button type="button" aria-label="Previous comparison">←</button><button type="button" aria-label="Next comparison">→</button></div></div>`;
  ...
  comparisonCards.forEach(card => {
    ...
    const fit = () => {
      ...
      if (width && height) card.style.setProperty('--comparison-ratio', width / height);
    };
  ...
  let distance = 0, start = 0, offsets = [], current = 0, frame = 0;
  function update() {
    frame = 0;
    const x = motion.matches ? Math.max(0, Math.min(distance, window.scrollY - start)) : viewport.scrollLeft;
    if (motion.matches) track.style.transform = `translate3d(${-x}px,0,0)`;
    current = offsets.reduce((best, offset, i) => Math.abs(offset - x) < Math.abs(offsets[best] - x) ? i : best, 0);
  ...
  function measure() {
    ...
    offsets = comparisonCards.map(card => Math.min(distance, card.offsetLeft - comparisonCards[0].offsetLeft));
    gallery.style.height = motion.matches ? `${innerHeight + distance}px` : '';
    start = gallery.getBoundingClientRect().top + scrollY;
    update();
  }
  function go(step) {
    const x = offsets[Math.max(0, Math.min(offsets.length - 1, current + step))];
    if (motion.matches) window.scrollTo({ top: start + x, behavior: 'smooth' });
    else viewport.scrollTo({ left: x, behavior: motion.matches || !matchMedia('(prefers-reduced-motion: reduce)').matches ? 'smooth' : 'auto' });
  }
  buttons.forEach((button, i) => button.addEventListener('click', () => go(i ? 1 : -1)));
...
if (caseNav) {
  ...
  const collapseNav = () => {
    ...
    collapseTimer = window.setTimeout(() => caseNav.classList.add("is-collapsed"), 1000);
  };
  ...
  if (caseLayout && "IntersectionObserver" in window) {
    ...
    }, { rootMargin: "0px 0px -35%", threshold: .05 });
